...
In [model.py](./model.py), we added a learnable embedding layer:
```python
...
In [train.py](./train.py), we implemented random label dropout to enable joint training of conditional and unconditional diffusion models:
```python
...
In [diffusion.py](./diffusion.py), the reverse sampling process is adjusted to perform a dual forward pass when a guidance scale $w > 0.0$ is provided:
```python
...
All results are saved in the [results/](./results/) directory.

### 1. Training Loss Curve
The model was trained for 15 epochs on MNIST. The loss history is documented below:
* Check out the [loss_curve.png](./results/loss_curve.png)
...
* View the [conditional_grid.png](./results/conditional_grid.png)
...
* View the [guidance_scale_sweep.png](./results/guidance_scale_sweep.png)
...
* View the [unconditional_vs_cfg.png](./results/unconditional_vs_cfg.png)
