...
class Diffusion:
    ...
    def sample(
        ...
        Args:
            ...
            labels:
                Conditioning labels (optional).
            cfg_scale:
                Guidance scale (w) for Classifier-Free Guidance.
            x_start:
                Pre-defined starting noise tensor (optional).

        Returns:
            Tensor of generated images in [0, 1].
        """
        model.eval()
        ...
        for timestep in reversed(
            ...
            if labels is not None and cfg_scale > 0.0:
                ...
                # Unconditional prediction (using the null class, which is model.num_classes)
                null_labels = torch.full_like(labels, model.num_classes)
                predicted_noise_uncond = model(x, t, null_labels)
                
                # Combine using CFG equation: eps = (1 + w) * eps_cond - w * eps_uncond
                predicted_noise = (1.0 + cfg_scale) * predicted_noise_cond - cfg_scale * predicted_noise_uncond
            else:
                # Fallback to normal prediction (which handles c=None internally)
        ...
        # Convert from [-1, 1]-ish range back into [0, 1] for visualization.
        x = (
            x.clamp(-1, 1)
            + 1
        ) / 2
